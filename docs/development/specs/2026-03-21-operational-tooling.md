...
> **Note**: Shell completions are not included in this spec — Cobra provides these out of the box via its built-in `completion` subcommand, which is already available to all GTB-based tools.
...
pkg/cmd/root/
├── root.go            ← MODIFIED: register doctor command, --output flag
```
